...
Public pages follow: problem → .cra input → compiler → typed IR → native runtime → generated artifacts → Android/iOS usage → measured evidence. Focused components in `features/product` render typed definitions from `content/product`. Reusable SVG assets live in `assets/visuals` and are generated by `scripts/generate-visuals.mjs`. Local screenshots use sized AVIF/WebP variants with provenance in `design/visual-sources.md`.

BenchmarkRepository normalizes both raw schemas and derives statistics from successful samples using the actual harness quantile floor((n−1)×q). Responsive SVG chart components consume this domain. Raw samples remain available through a lazy-loaded distribution and table. Home loads benchmarks only near the viewport. Route modules stay lazy.

Motion tokens are centralized in `app/motion.ts`; `MotionConfig reducedMotion="user"` wraps the application. The compiler story uses normal scrolling with a sticky desktop panel and static mobile steps. Public styles are scoped in `product.css`. Documentation registry, content, search, TOC, references and sidebars remain independent. Shared navigation exposes Examples and keeps tablet/mobile navigation and documentation search accessible.
